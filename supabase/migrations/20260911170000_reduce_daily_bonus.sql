...
BEGIN
  ...
  SELECT last_login_date, login_streak, balance INTO _last, _streak, _bal
  ...
  IF _last = _today THEN
    ...
    RETURN;
  END IF;
  IF _last = _today - 1 THEN _streak := COALESCE(_streak,0) + 1;
  ELSE _streak := 1;
  END IF;
  _reward := LEAST(5, GREATEST(1, _streak));
  UPDATE public.profiles SET balance = balance + _reward, login_streak = _streak,
    last_login_date = _today, updated_at = now() WHERE id = _uid;
